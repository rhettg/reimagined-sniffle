...
class ContentItemsController < ApplicationController
  ...
  def create
    ...
    if @content_item.save
      ...
      render json: @content_item.as_json.merge(type: @content_item.type), status: :created, location: @content_item
    else
  ...
  def update
    if @content_item.update(content_item_params)
      @content_item.file.attach(params[:content_item][:file]) if params[:content_item][:file].is_a?(ActionDispatch::Http::UploadedFile)
      render json: @content_item.as_json.merge(type: @content_item.type), status: :ok, location: @content_item
    else
      render json: { errors: @content_item.errors }, status: :unprocessable_entity
    end
  end
